...
class Google_Maps_Custom {
	...
	public function __construct() {
		...
		$this->taxonomy_name = 'gmcpt_rok';

		add_action( 'init', array($this,'add_google_maps_postype') );

		add_action( 'init', array($this, 'add_google_maps_rok') );

		add_action( 'init', array($this, 'add_google_maps_category') );
	...
	private function define_public_hooks() {
        ...
        $this->loader->add_action( 'gmcpt_display_areas', $plugin_public, 'get_all_points' );
		...
		register_post_type( $this->post_type_name,
			array(
				'labels' => array(
					...
					'add_new' => __( 'Dodaj nowe miejsce' ),
				),
				'public' => true,
				'has_archive' => false,
				'hierarchical' => true,
				'rewrite' => array('slug' => 'gmcpt'),
				'taxonomies' => array('gmcpt-rok', 'gmcpt-kategoria'),
				'supports' => array('title', 'editor', 'thumbnail' , 'page-attributes'  )
			)
		);
	}

	public function add_google_maps_rok() {
		// create a new taxonomy
		register_taxonomy(
			'gmcpt-rok',
			$this->post_type_name,
			array(
				'label' => __( 'Miejsce - Rok' ),
				'rewrite' => array( 'slug' => 'gmcpt-rok' ),
				'hierarchical' => true
			)
		);
	}

	public function add_google_maps_category() {
		// create a new taxonomy
		register_taxonomy(
			'gmcpt-kategoria',
			$this->post_type_name,
			array(
				'label' => __( 'Kategoria Miejsca' ),
				'rewrite' => array( 'slug' => 'gmcpt-kategoria' ),
				'hierarchical' => true
			)
		);
	}
